...
|---|---|---|---|
| 2026-09-13 | Phase 4.2.2 | Added post-generation safety as a separate graph node before final response publication; drafts are evaluated under `safety-post-generation-v1`, medication-change language routes to review, diagnosis/prescribing language blocks, grounding signals are required, and only redacted safety metadata is audited | Stop for review before Phase 4.3 persisted review queue and actions |
